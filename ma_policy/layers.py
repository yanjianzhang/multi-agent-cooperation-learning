...
def layer_norm_func(input_tensor, name=None, begin_norm_axis = -1):
    """Run layer normalization on the last dimension of the tensor."""
    return tf.keras.layers.LayerNormalization(name=name, axis=begin_norm_axis, epsilon=1e-12, dtype=tf.float32)(input_tensor)
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        if post_sa_layer_norm:
            with tf.compat.v1.variable_scope('post_a_layernorm'):
                x = layer_norm_func(x, begin_norm_axis=3)
        if n_mlp > 1:
            mlp = x
